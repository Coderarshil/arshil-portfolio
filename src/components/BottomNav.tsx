      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[var(--bg-primary)]/90 backdrop-blur-xl border-t border-[var(--border-color)]/20 pb-safe z-50 shadow-[0_-8px_30px_rgba(0,0,0,0.04)]">
        <div className="relative grid grid-cols-5 items-center h-16">
          {items.slice(0, 2).map(({ label, icon: Icon, target }, index) => (
            <button key={label} onClick={() => go(target)} className={`flex flex-col items-center justify-center w-full h-full gap-1 text-[var(--text-muted)] hover:text-[var(--accent-primary)] transition-colors ${index === 0 ? 'col-start-1' : 'col-start-2'}`}>
          ...
          {items.slice(2).map(({ label, icon: Icon, target }, index) => (
            <button key={label} onClick={() => go(target)} className={`flex flex-col items-center justify-center w-full h-full gap-1 text-[var(--text-muted)] hover:text-[var(--accent-primary)] transition-colors ${index === 0 ? 'col-start-4' : 'col-start-5'}`}>
              <Icon size={21} /><span className="text-[10px] font-medium">{label}</span>
            </button>
          ))}
        </div>
